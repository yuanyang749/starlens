...
> 当前工程参考。最后按 `skills/starlens/`、`apps/cli/src/install-mcp/` 和
> `packages/agent-tools/` 核对：2026-07-11。

## 1. 当前定位

Starlens 对 agent 和 IDE 的支持分为三条明确路径：
...
- 托管 HTTP MCP
  - 使用 `https://starlens.520ai.xin/mcp`，不启动本地进程
  - 推荐给连接 Starlens 托管服务的 Codex、opencode、Claude Code、Cursor 等 MCP 客户端
- 自部署 stdio MCP
  - 本地 stdio server 位于 `apps/mcp`，通过 Token 调用用户自己的 Starlens API
  - 适合自部署实例、内网 API 或不支持远程 HTTP MCP 的客户端
...
`stars setup` 通过 `npx skills add` 安装 Skill，再配置 MCP。只需要 Skill 时直接运行：

```bash
npx skills add https://github.com/yuanyang749/starlens
```

Skill 已安装、只需要 MCP 时运行 `stars install-mcp`。
...
连接托管服务时不需要运行这个本地进程。请使用 `stars setup --hosted` 或
`stars install-mcp --hosted` 写入 HTTP MCP 配置；完整的客户端配置示例见
`https://starlens.520ai.xin/docs/integrations`。
...
- `suggest_organization`
  - 知识整理建议（重复 / 过时 / 未分类）
- `get_sync_summary`
  - 按时间戳推断最近同步的新增 / 移除摘要；`added` 可能包含元数据更新，`changed` 当前不做精确区分
...
- 自部署 MCP server 是本地 stdio 进程，不额外开放公网端口；托管 HTTP MCP 复用现有 Web 服务的 `/mcp` 端点。
