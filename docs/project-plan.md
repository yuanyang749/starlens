...
- `apps/mcp`
  - 本地 stdio MCP server
  - 面向 Cursor、Hermes、OpenClaw 等 agent / IDE 暴露 Starlens 工具
- `apps/cli`
  - 命令行工具
  - 通过个人 token 调用后端 API
- `packages/core`
...
- Web 与 API 统一部署到 Vercel
- 数据库存储使用 PostgreSQL
- 优先使用 `Vercel + Neon Free` 起步
...
通过 Bearer Token 访问同一套 HTTP API，支持：
...
- AI：支持 Gateway、OpenAI-compatible、Anthropic、Gemini
