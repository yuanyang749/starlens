#!/usr/bin/env node
// 同步 skills/starlens/ → apps/cli/skills/starlens/
// 用法: node scripts/sync-skill.mjs
//
// 背景：apps/cli 会真实发布到 npm（@starlens-app/cli），npm 打包时只能收录
// 包目录（apps/cli/）内部的文件，仓库根目录的 skills/ 打不进去。
// 所以：
//   - skills/starlens/ 是唯一手写源文件（SKILL.md、references/、agents/）。
...
const SOURCE_DIR = join(ROOT, "skills", "starlens");
...
async function main() {
  ...
  console.log("✓ 已同步 skill 文件: skills/starlens/ → apps/cli/skills/starlens/");
}
