// star_repo 路由入口 —— 真实调用 GitHub star API（区别于 [id] 路由 PATCH isFavorite 那种本地标记）
// 不挂在 /repos/[id] 下：star 一个从未收藏过的仓库时，输入是任意 owner/repo，不是已知的 Starlens id。
...
import { GithubStarError, starRepoOnGithubForUser } from "@starlens/server/server/repos/github-star";
...
export async function POST(request: Request) {
  ...
  try {
    const repo = await starRepoOnGithubForUser(user.id, body.repo.trim());
    ...
    console.warn(`[routes/repos/star] starRepoOnGithubForUser failed: userId=${user.id} error=${msg}`);
    ...
    return fail("star_failed", msg, 500);
  }
}
