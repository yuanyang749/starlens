// AI 任务推荐路由入口
// 职责：鉴权、参数校验、运行时配置装配、限流、调用 recommendForTask 业务逻辑
...
import { recommendForTask } from "@starlens/server/server/ai/recommend";
...
export async function POST(request: Request) {
  ...
  const result = await recommendForTask(user.id, { taskDescription, limit }, chatConfig);

  // 透传限流状态到 meta.rateLimit。
