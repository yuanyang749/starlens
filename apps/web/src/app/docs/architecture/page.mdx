...
| AI 服务 | 支持摘要、重排、自然语言问答、SSE 多轮对话和模型配置 |
...
5. AI 问答接口先检索候选仓库，再按默认 AI Provider 生成回答或解释候选结果；多轮聊天接口通过 SSE 发送执行事件和最终答案，并持久化会话历史。
...
| `sync_runs` | 分页同步的进度、累计统计、错误和最近任务历史；中断后可续跑 |
...
| `deepseek_native` | `https://api.deepseek.com` | 使用 Bearer API Key 请求 OpenAI 风格模型列表 |

Provider 验证会尝试获取模型列表。成功时写入 `lastValidationStatus = success`，失败时写入 `error` 和错误信息，但不会把密钥返回给客户端。

当前问答运行时只消费 `openai_compatible` Chat Completions 配置。Native 类型用于配置管理和模型列表验证；调用兼容端点时应使用 `openai_compatible` 类型。
