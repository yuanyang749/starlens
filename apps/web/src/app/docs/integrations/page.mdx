...
```bash
curl "http://localhost:3000/api/search?q=vector&page=1&pageSize=20&sort=relevance" \
  -H "Authorization: Bearer stl_xxx"
```

更新仓库备注和重点收藏：

```bash
curl -X PATCH "http://localhost:3000/api/repos/{repoId}" \
  -H "Authorization: Bearer stl_xxx" \
...
```bash
curl -X POST "http://localhost:3000/api/repos/{repoId}/tags" \
  -H "Authorization: Bearer stl_xxx" \
  -H "Content-Type: application/json" \
  -d '{"tag":"rag"}'
```
...
MCP server 位于 `apps/mcp`，通过 stdio 暴露 Starlens 工具。它不重新实现业务逻辑，只通过 `@starlens/agent-tools` 调用 HTTP API。
