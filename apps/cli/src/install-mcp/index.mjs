...
//
// 两个入口:
//   runSetupWizard:     编排层 = npx skills add 装 skill + MCP 配置(一站式)
//   runInstallMcpWizard: 只配 MCP(跳过 skill 安装)
//
// 修复汇总：
...
//  #14 支持 --token-stdin 与 STARLENS_TOKEN 环境变量
//  #18 isGlobalInstall 用 realpathSync，更稳健
//  重构: skill 安装从 CLI 内部复制改为 npx skills add(agentskills.io 标准)
...
// Starlens GitHub 仓库 URL,npx skills add 的目标
...
function parseWizardArgs(args) {
  ...
  return { tokenArg, clientArg, tokenStdinFlag, hostedArg, localArg, langArg };
}

// 共享语言解析
async function resolveLocale(langArg) {
...
async function selectClients(clientArg, ui) {
  ...
  console.log("");
  const clients = await wizardCheckbox(CLIENT_ITEMS, ui);
  console.log(`Selected: ${clients.map((c) => CLIENT_LABELS[c]).join(", ")}`);
  return clients;
}

// MCP 配置核心逻辑(部署模式 + token + 写入配置 + 验证)
// 返回 true 表示配置了 MCP,false 表示跳过。
async function configureMcp(rl, ctx) {
  const { t, clients, config, env, tokenArg, tokenStdinFlag, hostedArg, localArg, cwd } = ctx;
  ...
  const doMcp = await wizardPrompt(
    rl,
    t.mcpConfigurePrompt.replace("{0}", [...mcpClients, ...otherClients].map((c) => CLIENT_LABELS[c]).join(", ")),
    "N",
  );
  if (!/^y$/i.test(doMcp)) {
...
// setup 编排入口: npx skills add 装 skill + MCP 配置
export async function runSetupWizard(args, config, env) {
  const parsed = parseWizardArgs(args);
  const locale = await resolveLocale(parsed.langArg);
  ...
  let rl;
  try {
    // Step 1: 安装 Skill via npx skills add (agentskills.io 标准方式)
    console.log("─".repeat(40));
    console.log(t.installingSkill);
    const skillOk = await spawnCommand("npx", ["skills", "add", GITHUB_REPO_URL]);
    if (!skillOk) {
      console.log(t.skillInstallFailed.replace("{0}", "npx skills add").replace("{1}", t.skillInstallFailedReason));
      console.log(t.skillManualHint.replace("{0}", `npx skills add ${GITHUB_REPO_URL}`));
    }

    // Step 2: 客户端选择(用于 MCP 配置)
    const clients = await selectClients(parsed.clientArg, ui);
    ...
    await configureMcp(rl, {
      t, clients, config, env,
      tokenArg: parsed.tokenArg,
      tokenStdinFlag: parsed.tokenStdinFlag,
      hostedArg: parsed.hostedArg,
      localArg: parsed.localArg,
      cwd,
    });
...
// install-mcp 入口: 只配 MCP(跳过 skill 安装)
export async function runInstallMcpWizard(args, config, env) {
  const parsed = parseWizardArgs(args);
  const locale = await resolveLocale(parsed.langArg);
  const t = messages[locale];
  const ui = checkboxLabels[locale];
  ...
  console.log("");
  console.log(t.mcpWizardTitle);
  console.log("═".repeat(40));
  console.log(t.mcpWizardIntro);
  console.log("");
